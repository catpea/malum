export default class Malum {
  constructor({arg}={arg:false}) {
        this.chain = [];

        // Create a function that will be our proxy target
        const chainFunction = (options, ...rest) => {
            ...
            if (this.chain.length > 0) {
                ...
                if (typeof lastItem === 'string') {
                  ...
                  // base
                  const data = { name: lastItem, opt: options }

                  // if args is enabled
                  if(arg) data.arg = [options, ...rest];
        ...
        const proxy = new Proxy(boundFunction, {
            get: (target, prop) => {
              ...
              if (prop === Symbol.asyncIterator) {
                ...
                if (prop === 'toObjects') {
                  return () => [...this.chain].map(o=>{

                    if(typeof o === 'string'){
                      const data = {name:o, opt:{}};
                      if(arg) data.arg = [];
                      return data;
                    }else{
                      return o;
